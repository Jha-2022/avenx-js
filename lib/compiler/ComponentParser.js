...
class ComponentParser {
    ...
    parse(filePath, type = 'component') {
        ...
        template = template.replace(/<([A-Z][a-zA-Z0-9]*)\s*([\s\S]*?)\s*\/>/g, (match, compName, attrsStr) => {
            const props = [];
            // Parse attributes using a quote-agnostic regex
            ...
            return `<div data-avenx-comp="${compName}"${propsAttr}></div>`;
        });
